  ...
  return (
    ...
    >
      <div className="   aspect-square sm:w-1/5 w-1/3  lg:w-full rounded-xl relative overflow-hidden">
        ...
        {p.tag && (
          <div className="bgrad absolute rounded-full text-white px-2 uppercase top-2 left-4 text-xs ">
            {p.tag}
          </div>
        )}
      </div>
    ...
    <div className="container mx-auto max-w-2xl p-4 text-black dark:text-white">
      {showHeading && (
        <div className=" flex items-center justify-between">
          <div className="text-[14px] w-full text-center md:w-fit md:text-left font-bold opacity-60 tracking-[0.2em]">
            FEATURED PROJECTS
          </div>
          <a
            href="/work"
            className="bg-gradient-to-r hidden md:block bg-clip-text text-transparent from-[#1584EB] to-[#24D6D9]"
          >
            All Work
          </a>
        </div>
      ...
      <div className="grid md:grid-cols-3 grid-cols-1 gap-2 mt-2">
        {showHeading &&
          projectList
            ...
            .map((p, ind) => <Project key={ind} p={p} />)}

        {!showHeading &&
          projectList.map((p, ind) => <Project key={ind} p={p} />)}
      </div>
      <div className="w-full mt-4 md:hidden  flex items-center">
        <a
          href="/work"
          className="bg-gradient-to-r text-center w-full bg-clip-text text-transparent from-[#1584EB] to-[#24D6D9]"
        >
          All Work
        </a>
      </div>
